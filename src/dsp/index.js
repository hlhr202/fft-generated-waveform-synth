 ...
 */
 const DSP = {
  ...
  DSP.deinterleave = (function() {
    let left; let right; let mix; const deinterleaveChannel = []; 
  ...
  FFT.prototype.forward = function(buffer) {
    ...
    if (2**k !== bufferSize) { throw "Invalid buffer size, must be a power of 2."; }
  ...
  FFT.prototype.inverse = function(real, imag) {
    ...
    const buffer = new Float64Array(bufferSize); // this should be reused instead
    for (i = 0; i < bufferSize; i++) {
  ...
  RFFT.prototype.forward = function(buffer) {
    const n         = this.bufferSize; 
        ...
        let i         = n >>> 1;
        const bSi       = 2 / n;
        let n2; let n4; let n8; let nn; 
        let t1; let t2; let t3; let t4; 
        let i1; let i2; let i3; let i4; let i5; let i6; let i7; let i8; 
        let st1; let cc1; let ss1; let cc3; let ss3;
        let e; 
        let a;
        let rval; let ival; let mag; 
  ...
  function Sampler(file, bufferSize, sampleRate, playStart, playEnd, loopStart, loopEnd, loopMode) {
    ...
    const audio = /* new Audio(); */ document.createElement("AUDIO");
    const self = this;
   
    this.loadSamples = function(event) {
      const buffer = DSP.getChannel(DSP.MIX, event.frameBuffer);
  ...
  Sampler.prototype.generate = function() {
    ...
    const playStartSamples = this.playStart * this.samples.length; // ie 0.5 -> 50% of the length
    const playEndSamples = this.playEnd * this.samples.length; // ie 0.5 -> 50% of the length
    let offset;
  ...
  function IIRFilter2(type, cutoff, resonance, sampleRate) {
    ...
    this.calcCoeff = function(cutoff, resonance) {
      ...
      this.damp = Math.min(2 * (1 - resonance**0.25), Math.min(2, 2/this.freq - this.freq * 0.5));
    };
  ...
  WindowFunction.Gauss = function(length, index, alpha) {
    return Math.E**(-0.5 * ((index - (length - 1) / 2) / (alpha * (length - 1) / 2))**2);
  };
  ...
  function Biquad(type, sampleRate) {
    ...
    this.coefficients = function() {
      const b = [this.b0, this.b1, this.b2];
      const a = [this.a0, this.a1, this.a2];
      return {b, a};
    };
    ...
    this.recalculateCoefficients = function() {
      ...
      const w0 = DSP.TWO_PI * this.f0 / this.Fs;
  ...
  DSP.mag2db = function(buffer) {
    const minDb = -120;
    const minMag = 10.0**(minDb / 20.0);
  
    const {log} = Math;
    const {max} = Math;
   
    const result = Float64Array(buffer.length);
    for (let i=0; i<buffer.length; i++) {
  ...
  DSP.freqz = function(b, a, w) {
    ...
    const result = Float64Array(w.length);
  ...
  function GraphicalEq(sampleRate) {
    ...
    this.recalculateFilters = function() {
      const bandCount = Math.round(Math.log(this.maxFreq/this.minFreq) * this.bandsPerOctave/ Math.LN2);
  
      this.filters = [];
      for (let i=0; i<bandCount; i++) {
        const freq = this.minFreq*(2**(i/this.bandsPerOctave));
        const newFilter = new Biquad(DSP.PEAKING_EQ, this.FS);
  ...
  MultiDelay.prototype.process = function(samples) {
    ...
    for (let i=0; i<samples.length; i++) {
      ...
      const delaySample = (this.delayBufferSamples[this.delayOutputPointer] === null ? 0.0 : this.delayBufferSamples[this.delayOutputPointer]);
  ...
  Reverb.prototype.process = function (interleavedSamples){ 
    ...
    const leftRightMix = DSP.deinterleave(interleavedSamples);
    this.LOWPASSL.process( leftRightMix[DSP.LEFT] );
    this.LOWPASSR.process( leftRightMix[DSP.RIGHT] ); 
    const filteredSamples = DSP.interleave(leftRightMix[DSP.LEFT], leftRightMix[DSP.RIGHT]);
